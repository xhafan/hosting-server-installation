...
(crontab -l ; echo "0 23 * * * docker run --rm --name certbot -v /docker-volumes/etc/letsencrypt:/etc/letsencrypt -v /docker-volumes/var/lib/letsencrypt:/var/lib/letsencrypt -v /docker-volumes/data/letsencrypt:/data/letsencrypt -v /docker-volumes/var/log/letsencrypt:/var/log/letsencrypt certbox-no-pax renew --webroot -w /data/letsencrypt --quiet && docker restart nginx") | sort - | uniq - | crontab - # trying to renew SSL certificate every day at 23h (https://www.humankode.com/ssl/how-to-set-up-free-ssl-certificates-from-lets-encrypt-using-docker-and-nginx)
